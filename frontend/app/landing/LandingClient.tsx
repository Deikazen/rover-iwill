...
export default function LandingClient({ initialItems }: LandingClientProps) {
  ...
  const [selectedItem, setSelectedItem] = useState<RoverItem | null>(
    initialItems[0] || null,
  );
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesSearch =
        ...
        (item.sub_judul?.toLowerCase() || "").includes(
          searchQuery.toLowerCase(),
        ) ||
        (item.deskripsi?.toLowerCase() || "").includes(
          searchQuery.toLowerCase(),
        );
  ...
  const handleModalSubmit = async (payload: {
    ...
    if (modalType === "create") {
      try {
        ...
        if (!response.ok)
          throw new Error("Gagal menyimpan ke database backend");
      ...
      try {
        ...
        if (!response.ok)
          throw new Error("Gagal memperbarui data di database backend");
        const updatedItem = await response.json();
        setItems((prev) =>
          prev.map((it) => (it.id === selectedItem.id ? updatedItem : it)),
        );
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Apakah Anda yakin ingin menghapus data rover ini dari database misi?",
      )
    )
      return;
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Gagal menghapus data di database backend");
    ...
    <div className="min-h-screen bg-[#030305] text-zinc-100 font-sans selection:bg-[#F97316] selection:text-white relative overflow-hidden">
      ...
      {actionStatus && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-xl border backdrop-blur-xl animate-fade-in shadow-[0_10px_30px_rgba(0,0,0,0.5)] border-zinc-800 bg-[#0c0c10]/90">
          <div
            className={`w-3 h-3 rounded-full ${actionStatus.type === "success" ? "bg-emerald-500 shadow-[0_0_10px_#10b981]" : "bg-rose-500 shadow-[0_0_10px_#f43f5e]"}`}
          />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-zinc-300 font-mono tracking-wider">
                MARS MISSION COMMAND
              </p>
              <p className="text-xs text-zinc-500 font-mono">
                Dibuat menggunakan Next.js & FastAPI backend
              </p>
            </div>
          </div>
